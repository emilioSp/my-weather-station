# 2026-09-01-temperature-units-v2: Browser-persisted temperature units
...
- in general all files in web workspace
...
- postcondition: A Chromium browser loads the web app, selects Fahrenheit, and the test observes Fahrenheit temperature, dew point, and heat index values in current-reading cards and the chart UI, with no Celsius unit shown for these values.
...
- probe: `npm run test -w @wx/web -- e2e/temperature-units.spec.ts`
- postcondition: A Chromium browser selects Fahrenheit, reloads the web app, and the test observes the selected Fahrenheit control, Fahrenheit values, and the `fahrenheit` localStorage value.
...
- probe: `npm run test -w @wx/web -- e2e/temperature-units.spec.ts`
- postcondition: A Chromium browser with no saved unit and with an invalid saved unit both load the web app with Celsius selected and Celsius values displayed.
